...
/* Restituisce { labels, pages, speeds, days, label, totalPages, totalSeconds } */
function getPeriodInfo() {
    const allData = getAllDailyData();
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    let labels = [], pages = [], speeds = [], days = [], label = "";
    ...
    if (statsMode === "week") {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const h = (entry.seconds || 0) / 3600;
            speeds.push(h > 0 && (entry.pages || 0) > 0 ? +((entry.pages || 0) / h).toFixed(1) : null);
        }
        ...
        for (let m = 0; m < 12; m++) {
            ...
            pages.push(mPages);
            const h = mSec / 3600;
            speeds.push(h > 0 && mPages > 0 ? +(mPages / h).toFixed(1) : null);
        }
        label = String(year);
    }
    ...
    return { labels, pages, speeds, days, label, totalPages, totalSeconds };
}
...
function renderCharts(info) {
    ...
    pagesChartInst = new Chart(pagesCtx, {
        type: 'bar',
        data: {
            labels: info.labels,
            datasets: [{
                data: info.pages,
                backgroundColor: 'rgba(92,44,22,0.55)',
                borderColor:     '#5c2c16',
                borderWidth: 1.5,
                borderRadius: 6,
                borderSkipped: false
    ...
    speedChartInst = new Chart(speedCtx, {
        type: 'line',
        data: {
            labels: info.labels,
            datasets: [{
...
function saveEditRow(key) {
    ...
    if (btn) { btn.textContent = "✓ Salvato"; setTimeout(() => btn.textContent = "Salva", 1400); }
}
